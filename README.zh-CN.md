...
**看扩展名（或文件名）决定开不开。** 不在下面的文件**完全不会被打开**。
...
没列到的（例如 `.md`、图片、二进制文件）不扫。`.md` 是刻意的：文档里提到 `api.openai.com` 是常态，扫了只会满江红。

三条线索的覆盖不一样：

| 线索 | 覆盖 |
|---|---|
| 密钥样式 | **与语言无关**，上面每一种文件都扫。只认四种前缀：Anthropic、OpenAI（新旧两种）、Google |
| AI 域名 | **与语言无关**，七个域名 |
| SDK 引用 | 认五个关键字：`import` / `from` / `require` / `using` / `use`，**忽略大小写**。涵盖 Python、JS／TS、Go、Ruby、Java、Kotlin、Swift、Scala、Dart（`import`）、C#／VB.NET（`using`）、Rust／PHP（`use`） |
| 依赖清单 | 上面那一列都认，一律列为低风险（「装了，不确定在不在用」） |

引用那一行与依赖清单里，厂商名是用**子字符串**比对的，所以 Rust 的 `async_openai`、Swift 的 `OpenAIKit`、
C# 的 `Anthropic.SDK`、Maven 的 `openai-java` 都抓得到。只在这两处放宽，别的地方仍是整词比对。
