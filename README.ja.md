...
このツールはリポジトリを 1 つ端から端まで読み、AI を使っている行をすべて並べ、それぞれがどれくらい急ぎで、
...
パスが分からない？そのフォルダを**ターミナルのウィンドウにドラッグ**すれば、勝手に入力されます。
...
一覧は 3 段階に分かれます。見た目のためではなく、対応する人が 3 種類いるからです。
...
本人の目にはそれはただのコーディングだからです。見るのは機械が残した痕跡、3 か所：
...
はっきり言います。これはキーワード照合であって、深い解析ではありません。全体を走らせるたびに、見えない 4 点を末尾に自分で書き出します。

1. **自前でラップされた AI 呼び出し。** コードには `from vendor.llm_client import ask` としかなく、その先がどのサービスかは見えません。
   たとえば 2 年前に誰かが社内モジュールを書き、その人は辞め、今ではどこに繋がっているのか誰も覚えていない。
...
リポジトリに `vendor/` や `internal/` があれば、そこは人が開いて確認する場所です。

最初の 2 点は手抜きではありません。見通すにはファイルをまたぐデータフロー解析が要り、そうなるともう、半日で書けるツールではありません。
...
| Linux | 普段のターミナル | `python3 --version` | たいてい入っています。なければ `sudo apt install python3`（Ubuntu／Debian。他のディストリビューションは各自のパッケージマネージャーで） |
...
- 文字化けする？ウィンドウで直接走らせる分には化けません——ツール自身が UTF-8 で出力します。化けるのは、結果をファイルに保存したり
  別のプログラムに渡してから見たときです。その場合は Windows で先に `chcp 65001` を打ってください。今のウィンドウを UTF-8 に切り替えるコマンドで、
  65001 は Windows が UTF-8 に付けている番号です。Windows にしかなく、macOS と Linux には不要です。
- 化けるのではなく□が出る？古いコンソールのフォントにその文字がないだけです。フォント設定をいじらず、**Windows ターミナル**に替えてください。
...
⚠ 出力には鍵の断片が含まれます。スクリーンショットを撮ったり人に貼り付けたりする前に、一度目を通してください。
...
四半期ごとの棚卸しよりずっと安上がりです。「高」があれば終了コード 1、なければ 0。引数の誤りやパス不在は 2。
...
`.md` は意図的に対象外です。ドキュメントに `api.openai.com` が出てくるのは普通のことで、ノイズだらけになります。2 MB を超えるファイルは飛ばします。
...
そしてなぜ「禁止」の通達では止まらないのか——カメラが一台もない路地に「駐車禁止」の札を貼るようなものだからです。
...
まずはこのリポジトリ 1 つをスキャンしてください。冒頭の数字に戻って——まだ合っていると思いますか。
